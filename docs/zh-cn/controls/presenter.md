...
`Presenter` 是用于高级呈现的全宽布局控件。需要呈现图片、多个图标、插图、预览或其他组合视觉内容时使用它；同一行只放置一个 Presenter。只需要标题、单段正文和单个图标时使用 [Card](card.md)。

每个 Presenter 声明都应显式提供 `Title`、`Content`、`PresenterMode` 和 `PresenterPosition`。运行时回退值是 `Split` 与 `Right`，但显式声明能够让组合方式保持清晰一致。
...
| `PresenterPosition` | `PresenterPosition` | `Right` | Split 模式中的 Presentation 位置。 |
...
| 位置 | 排列 |
| --- | --- |
| `Right` | Title、Content 与 Body 位于左侧，Presentation 位于右侧；这是默认布局。 |
| `Left` | Presentation 位于左侧，Title、Content 与 Body 位于右侧。 |
...
`TopDown` 将 Presentation 放在上方，将 Title、Content 和 Body 放在下方并统一靠左。它适合宽幅预览、图表或需要在视觉内容之后继续说明和操作的布局。
...
[HeaderChunk](chunk.md#headerchunk) 是页面级 Presenter 特化。它使用相同的字段与三种模式，但具有强调背景、HeaderSize 标题和页面开头语义。HeaderChunk 与 Chunk 同级；普通 Presenter 则放在 Chunk 的 Body 中。
