...
`Button.Variant` 选择按钮的 Material Design 3 外观与视觉强调，而不是按钮的尺寸或布局。默认值是 `Outlined`。
...
一组操作通常只应有一个 `Filled` 按钮。破坏性意图使用 `Danger`；它是 Flourish 在五种 Material Design 3 强调变体之上增加的语义扩展。外部位置由包含按钮的布局容器控制；不要通过 `Variant` 选择结构尺寸。
