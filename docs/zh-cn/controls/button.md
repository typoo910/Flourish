...
当 `Content` 为 `null` 时，`IconButton` 使用无内边距的紧凑 `32 × 32` 布局。仅图标按钮应同时设置可见的 `ToolTip` 和可识别操作意图的 `AutomationProperties.Name`。字符串等简单 `ToolTip` 内容会自动包装为 `FlourishToolTip`，从而遵循 Shell 感知的定位规则。
