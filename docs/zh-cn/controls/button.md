...
当 `Content` 为 `null` 时，`IconButton` 使用无内边距的紧凑 `32 × 32` 布局。仅图标按钮应同时设置可见的 `ToolTip` 和可识别操作意图的 `AutomationProperties.Name`。由字符串创建的原生 WPF Tooltip 与显式 `FlourishToolTip` 会共享 Flourish Temporary Overlay 表面和 Shell 感知的定位行为；Button 系列控件在禁用状态下也会显示提示。
