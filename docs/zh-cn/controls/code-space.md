...
`CodeSpace` 在透明、圆角且带轻量描边的表面中呈现精确文本片段，并在右上角提供复制按钮。源代码或命令文本可能需要由读者复制时使用它。普通多段正文使用 [Document](document.md)，运行时输出或日志历史使用 [OutputCard](output-card.md)。
...
该表面与 Document 共用透明背景、带圆角且细而低对比度的边框、内边距和小顶部外边距。当控件位于 Chunk 标题或 Content 之后时，应保留此外边距。
