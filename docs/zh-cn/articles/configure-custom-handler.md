...
自定义内容不会自动启用所属 Shell 功能。应先在 [Shell 配置](shell-configuration.md)中启用对应区域：状态栏内容需要 `UseStatusBar()`，动态工具栏内容需要 `UseDynamicToolbar()`，标题栏内容需要 `UseTitleBar()`；`SetProfileContent` 还需要在标题栏中调用 `SetProfile()`。

区域的内置行为分别参见[动态工具栏](dynamic-toolbar.md)、[状态栏](status-bar.md)和[标题栏](configure-title-bar.md)。
