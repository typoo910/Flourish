...
如果一张卡片需要多个主题或标题，应将其拆成多个同级表面。操作产生动态响应文字时，操作应留在自己的表面中，并在旁边新增 `Output` Card 承载持续或原始输出，或者新增 `Result` Card 展示完成结果。Output 与 Result Card 推荐只保留 `Title`：省略 `Text`，在 `Body` 中先放一个简洁的 Description 角色文本，再放输出或状态内容。两个表面应位于同一个全宽 Chunk 中。

每个独立配置项使用一个 `ListCard`，相关 ListCard 纵向堆叠且每一行铺满所在列。连续行之间使用紧凑的 `FlourishListCardPeerMargin`，让整列形成相关组合。同一列中不能混用 `ListCard` 与任何其他卡片类型。每个 Chunk 优先采用仅含 ListCard 的单列布局；章节还需要展示输出时，同一 Chunk 可以增加另一列专门放置 `Output` 或 `Result` Card。
...
同一行的卡片必须保持相同高度。双列 ListCard 加 Output/Result 的组合中，完整 ListCard 列与相邻 Card 的整体高度必须一致。卡片换成多行时，行间距必须与列间距一致。应优先使用共享间距资源，或者使用能够强制统一间距的布局容器，避免为各张卡片设置彼此无关的 Margin。
...
4. 独立主题或行为已拆成独立表面；Output 与 Result Card 通常只保留 `Title`，说明与输出均放入 `Body`。
...
9. 同行卡片高度一致；ListCard 加 Output/Result 的双列整体高度也一致，且行间距与列间距一致。
10. 非交互信息、紧凑配置、整卡操作和视觉展示场景分别使用 `Card`、`ListCard`、`CardButton` 与 `IconCard`。
