...
每个独立配置项使用一个 `ListCard`，相关 ListCard 纵向堆叠且每一行铺满所在列。同一列中不能混用 `ListCard` 与任何其他卡片类型。每个 Chunk 优先采用仅含 ListCard 的单列布局；章节还需要展示输出时，同一 Chunk 可以增加另一列专门放置 `Output` 或 `Result` Card。
