...
每个主导航内容页面都使用 `PageBody` 作为根容器。其直接子项只能是 `HeaderChunk` 与 `Chunk`：标准页面以一个 HeaderChunk 开头，随后放置一个或多个 Chunk。HeaderChunk 最多出现一次且必须位于最前方，两种区块都全宽独占一行。
...
- `Split` 横向分隔两侧。默认文案与 Body 在左、Presentation 在右；`PresenterPosition="Left"` 交换两侧。
- `TopDown` 把 Presentation 放在上方，把 Title、Content 与 Body 放在下方并统一靠左。
- `Overlay` 让 Presentation 填满控件，并把文案与 Body 呈现在其上方。

Presentation 是默认 XAML 内容属性；Body 应通过显式的 `Presenter.Body` 属性元素赋值。普通 Presenter 全宽独占一行，文案与 Body 一侧保持透明并共用左侧对齐线；Presentation 区域使用浅灰色圆角背景、填满其分配空间，并让展示内容居中。
